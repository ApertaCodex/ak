...
// -------- CLI help --------
static void showLogo(std::ostream& out = std::cout) {
    ...
    // Use a stringstream to build the final output first.
    ...
    if (isColorSupported()) {
        ...
        buffer << "AK - Secret Management CLI\n"
               << "Fast • Secure • Human-Friendly\n\n";
    }

    // Print the entire buffered output in one operation.
    out << buffer.str();
}
