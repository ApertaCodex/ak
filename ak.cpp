...
SECRET MANAGEMENT:
  ...
  ak rm --profile <NAME>          Remove a profile
  ak search <PATTERN>             Search for secrets by name pattern (case-insensitive)
  ak cp <NAME>                    Copy secret value to clipboard
  ak purge [--no-backup]          Remove all secrets and profiles (creates backup by default)
...
EXPORT/IMPORT:
  ...
  ak import --profile|-p <p> --format|-f <fmt> --file|-i <file> [--keys]
...
EXAMPLES:
  ...
  ak import -p prod -f env -i .env --keys     # Import only known service keys
  ak rm --profile old-profile    Remove a profile
  ak purge --no-backup           Remove all data without creating backup
...
int main(int argc, char **argv)
{
    ...
    else if (cmd == "rm")
    {
        ...
        if (args.size() >= 3 && args[1] == "--profile")
        {
            ...
            // Remove profile file
            string profilePath = cfg.profilesDir + "/" + profileName;
    ...
    else if (cmd == "purge")
    {
        ...
        for (const auto& profile : profiles)
        {
            string profilePath = cfg.profilesDir + "/" + profile;
            if (fs::exists(profilePath))
                fs::remove(profilePath);
        }
