...
static bool test_one(const Config &cfg, const string &svc)
{
    ...
    if (svc == "perplexity")
        return curl_ok("-X POST https://api.perplexity.ai/chat/completions -H \"Authorization: Bearer " + k + "\" -H \"Content-Type: application/json\" -d '{\"model\":\"llama-3.1-sonar-small-128k-online\",\"messages\":[{\"role\":\"user\",\"content\":\"hello\"}],\"max_tokens\":4}'");
    if (svc == "sambanova")
    ...
    if (svc == "xai")
        return curl_ok("-X POST https://api.x.ai/v1/chat/completions -H \"Authorization: Bearer " + k + "\" -H \"Content-Type: application/json\" -d '{\"model\":\"grok-beta\",\"messages\":[{\"role\":\"user\",\"content\":\"hello\"}],\"max_tokens\":4}'");
    return false;
}
