...
// Per-character gradient with a diagonal phase shift per line
static void print_gradient_block(const std::array<string,6>& lines,
                                 const vector<RGB>& palette_stops) {
    // Compute max visible width to keep gradient consistent across lines
    size_t maxw = 0;
    for (auto& ln : lines) maxw = std::max(maxw, ln.size());
    if (maxw == 0) return;

    // Build a long gradient (with extra to allow diagonal shift)
    const size_t extra = 16; // small pad so shift doesn’t clip ends
    auto grad = make_gradient(palette_stops, maxw + extra);

    for (size_t row = 0; row < lines.size(); ++row) {
        const string& s = lines[row];
        size_t shift = row * 2; // diagonal feel: 0,2,4,6,...
...
static bool isColorSupported(); // your existing function
...
static void showLogo() {
    ...
    // Sleeker ASCII (kept your shape, fixed misaligned box in 2nd col)
    const std::array<string,6> LOGO = {
        "        ████████╗   ██╗  ██╗",
        "        ██╔══███║   ██║ ██╔╝",
        "        ██║██║██║   █████╔╝ ",
        "        ██║  ║██║   ██╔═██╗ ",
    ...
    // “Future” palette: neon magenta → electric blue → cyan
    ...
    print_gradient_block(LOGO, FUTURE);

    // Taglines: subtle two-stop sweep
    const vector<RGB> TAG = { {200,220,255}, {120,180,255} };
    const std::array<string,2> TL = {
        "    🔐 Secure Secret Management",
        "    ⚡ Fast • Secure • Human-Friendly"
    };

    // Print taglines with a gentle left→right gradient
    for (const auto& line : TL) {
        auto g = make_gradient(TAG, line.size());
        for (size_t i = 0; i < line.size(); ++i) {
            char ch = line[i];
            if (ch == ' ') { cout << ch; continue; }
            cout << ansi24(g[i]) << ch << reset();
        }
        cout << "\n";
    }
    cout << "\n";
