...
// -------- CLI help --------
// Generate gradient color for 256-color terminals
static string getGradientColor(int step, int totalSteps) {
    if (!isColorSupported()) return "";
    
    // Create a gradient from blue (21) to cyan (51) to green (46)
    // Using 256-color ANSI escape codes
    int color;
    if (step < totalSteps / 2) {
        // Blue to cyan transition (21 -> 51)
        float ratio = (float)step / (totalSteps / 2);
        int blueRange[] = {21, 27, 33, 39, 45, 51}; // Blue to cyan progression
        int idx = (int)(ratio * 5);
        if (idx >= 5) idx = 5;
        color = blueRange[idx];
    } else {
        // Cyan to green transition (51 -> 46)
        float ratio = (float)(step - totalSteps / 2) / (totalSteps / 2);
        int cyanGreenRange[] = {51, 50, 49, 48, 47, 46}; // Cyan to green progression
        int idx = (int)(ratio * 5);
        if (idx >= 5) idx = 5;
        color = cyanGreenRange[idx];
    }
...
static void showLogo() {
    ...
    // Detailed AK logo using block characters for better gradient effect
    ...
    cout << "\n";
    for (int i = 0; i < (int)logoLines.size(); i++) {
        string gradientColor = getGradientColor(i, (int)logoLines.size());
        cout << gradientColor << logoLines[i] << Colors::RESET << "\n";
    }
    cout << "\n";

    cout << colorize("    🔐 ", "") << colorize("Secure Secret Management", Colors::BRIGHT_WHITE + Colors::BOLD) << "\n";
    cout << colorize("    ⚡ ", "") << colorize("Fast • Secure • Human-Friendly", Colors::BRIGHT_GREEN) << "\n\n";
}
