...
namespace Colors { static const std::string RESET = "\033[0m"; }
bool isColorSupported();


static std::string colorize(std::string_view text, std::string_view color) {
    // Also check if the color is empty to avoid adding RESET codes unnecessarily.
    if (!isColorSupported() || color.empty()) {
        return std::string(text); // Return a new string constructed from the view
    }

    // Since we are creating a new string, the return type remains std::string.
    // We must construct strings from the views to concatenate them.
    return std::string(color) + std::string(text) + Colors::RESET;
}
