...
static void showLogo() {
    ...
    // Gradient colors (hex or ANSI sequences depending on your colorize implementation)
    std::vector<std::string> gradient = {
        Colors::BRIGHT_MAGENTA, Colors::MAGENTA, Colors::BRIGHT_BLUE,
        Colors::BLUE, Colors::BRIGHT_CYAN, Colors::CYAN
    };
    ...
    // Print each line with gradient cycling
    for (size_t i = 0; i < std::size(logoLines); i++) {
        std::string color = gradient[i % gradient.size()];
        cout << colorize(logoLines[i], color) << "\n";
    }

    cout << "\n"
         << colorize("    🔐 ", Colors::BRIGHT_WHITE)
         << colorize("Secure Secret Management", Colors::BRIGHT_WHITE + Colors::BOLD) << "\n"
         << colorize("    ⚡ ", Colors::BRIGHT_WHITE)
         << colorize("Fast • Secure • Human-Friendly", Colors::BRIGHT_GREEN) << "\n\n";
}
