...
{
    cout << "🎉 Welcome to AK – your friendly secret‑keeper CLI! 🎉\n"
            "🔐 Securely store, retrieve, and manage your secrets with ease.\n"
            "\n"
            "🚀 Usage:\n"
            "  ak help\t# 🎉 Show this friendly help message\n"
            "  ak backend\t# 🗄️ Show which storage backend is used (gpg or plain)\n"
            "  ak set <NAME>\t# 🔐 Store a secret (you’ll be prompted for the value)\n"
            "  get <NAME> [--full]\t# 🔍 Retrieve a secret (masked by default, use --full to see it all)\n"
            ...
            "  migrate exports <file>\t# 🔄 Migrate export file\n"
            "  profiles\t# 📂 List saved profiles\n"
            "  run --profile <p> -- <cmd...>\t# 🏃 Run command with environment\n"
            "  guard enable|disable\t# 🛡️ Enable/disable pre‑secret guard\n"
            "  test <service>|--all [--json] [--fail-fast]\t# 🧪 Test service connectivity\n"
            "  doctor\t# 🩺 System diagnostics\n"
            "  audit [N]\t# 📜 Show recent audit log entries\n"
            "  install-shell\t# ⚙️ Install shell auto‑load snippet\n"
            "  uninstall\t# 🗑️ Remove configuration and binary\n";
}
...
static bool test_one(const Config &cfg, const string &svc)
{
    ...
    if (svc == "azure_openai")
    {
        ...
        if (ep.empty())
            return false;
        return curl_ok("-H \"api-key: " + k + "\" \"" + ep + "/openai/deployments?api-version=2024-02-15-preview\"");
    }
    if (svc == "brave")
    ...
    if (svc == "perplexity")
        return curl_ok("-X POST https://api.perplexity.ai/chat/completions -H \"Authorization: Bearer " + k + "\" -H \"Content-Type: application/json\" -d '{\"model\":\"sonar-small-chat\",\"messages\":[{\"role\":\"user\",\"content\":\"hello\"}],\"max_tokens\":4}'");
    if (svc == "sambanova")
    ...
    if (svc == "xai")
        return curl_ok("-X POST https://api.x.ai/v1/chat/completions -H \"Authorization: Bearer " + k + "\" -H \"Content-Type: application/json\" -d '{\"model\":\"grok-1\",\"messages\":[{\"role\":\"user\",\"content\":\"hello\"}],\"max_tokens\":4}'");
    return false;
}
...
int main(int argc, char **argv)
{
    ...
    else if (cmd == "import")
    {
        ...
        if (file.empty())
            error(cfg, "Usage: ak import --profile <p> --format env|dotenv|json|yaml --file <file>");
        ifstream in(file);
        if (!in)
        ...
        ok(cfg, "Imported " + to_string(importedKeys.size()) + " keys into profile '" + prof + "'");
    ...
    {
        // remove config dir; try to remove binary if absolute
