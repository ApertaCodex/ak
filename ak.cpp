...
namespace AppUI {
    // Group UI-related constants in a namespace.
    namespace constants {
        ...
        constexpr std::string_view title = "    🔐 Secure Secret Management";
        constexpr std::string_view tagline = "    ⚡ Fast • Secure • Human-Friendly";
        constexpr std::string_view plain_title = "AK - Secret Management CLI";
    } // namespace constants

    // The function now takes the output stream as an argument.
    // It defaults to std::cout, so existing calls don't need to change.
    void showLogo(std::ostream& out = std::cout) {
        using namespace constants;
        std::stringstream buffer;

        if (isColorSupported()) {
            buffer << colorize(logo, Colors::BRIGHT_CYAN) << "\n"
                   << colorize(title, Colors::BRIGHT_WHITE + Colors::BOLD) << "\n"
                   << colorize(tagline, Colors::BRIGHT_GREEN) << "\n\n";
        } else {
            buffer << plain_title << "\n"
                   << tagline << "\n\n";
        }
...
// --- Example Usage ---
// AppUI::showLogo();         // Prints to the console (std::cout)
// AppUI::showLogo(std::cerr); // Prints to the standard error stream
