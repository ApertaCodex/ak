...
static void error(const Config &cfg, const string &msg, int code = 1)
{
    if (cfg.json)
        ...
        cerr << colorize("❌ " + msg, Colors::BRIGHT_RED) << "\n";
    exit(code);
}
static void ok(const Config &cfg, const string &msg)
{
    if (!cfg.json)
        cerr << colorize("✅ " + msg, Colors::BRIGHT_GREEN) << "\n";
}
static void warn(const Config &cfg, const string &msg)
{
    if (!cfg.json)
        cerr << colorize("⚠️  " + msg, Colors::BRIGHT_YELLOW) << "\n";
}
...
// ------- capability checks -------
static bool env_contains_utf8(const char* v) {
    if (!v) return false;
    string s(v);
    std::transform(s.begin(), s.end(), s.begin(), ::tolower);
    return s.find("utf-8") != string::npos || s.find("utf8") != string::npos;
}

static bool supports_unicode() {
    if (const char* forced = std::getenv("AK_ASCII")) { // allow manual override
        if (string(forced) == "1") return false;
    }
    if (env_contains_utf8(std::getenv("LC_ALL")) ||
        env_contains_utf8(std::getenv("LC_CTYPE")) ||
        env_contains_utf8(std::getenv("LANG"))) {
        return true;
    }
    return false;
}

static inline string ansi24(RGB c) {
    return "\x1b[38;2;" + std::to_string(c.r) + ";" + std::to_string(c.g) + ";" + std::to_string(c.b) + "m";
}
static inline string reset() { return "\x1b[0m"; }

static RGB lerp(const RGB& a, const RGB& b, double t) {
    auto L = [&](int x, int y){ return int(std::round(x + (y - x) * t)); };
    return { L(a.r,b.r), L(a.g,b.g), L(a.b,b.b) };
}

static vector<RGB> make_gradient(const vector<RGB>& stops, size_t steps) {
    vector<RGB> out; out.reserve(steps ? steps : 1);
    if (stops.empty()) return out;
    if (stops.size() == 1) { out.assign(steps, stops.front()); return out; }
    const size_t segs = stops.size() - 1;
    for (size_t i = 0; i < steps; ++i) {
        double u = segs * (steps > 1 ? (double(i) / (steps - 1)) : 0.0);
        size_t s = std::min<size_t>(size_t(std::floor(u)), segs - 1);
        double t = u - s;
        out.push_back(lerp(stops[s], stops[s+1], t));
    }
    return out;
}

static void print_per_char_gradient(const std::vector<string>& lines,
                                    const vector<RGB>& palette_stops) {
    size_t maxw = 0;
    for (auto& ln : lines) maxw = std::max(maxw, ln.size());
    if (maxw == 0) return;
    const size_t extra = 16;
    auto grad = make_gradient(palette_stops, maxw + extra);
    for (size_t row = 0; row < lines.size(); ++row) {
        const string& s = lines[row];
        size_t shift = row * 2;
        for (size_t i = 0; i < s.size(); ++i) {
            char ch = s[i];
            if (ch == ' ') { cout << ch; continue; }
            RGB c = grad[std::min(shift + i, grad.size()-1)];
            cout << ansi24(c) << ch << reset();
        }
        cout << "\n";
    }
}

// Your existing color capability check (kept)
static bool isColorSupported();

// ------- main logo printer -------
static void showLogo() {
    // Ensure C locale won’t squash multibyte on some libstdc++ builds
    std::setlocale(LC_ALL, "");

    const bool unicode_ok = supports_unicode();
    const bool color_ok = isColorSupported();

    if (unicode_ok && color_ok) {
        // Unicode + gradient (future vibe)
        ...
        const vector<RGB> FUTURE = {
            {255,  64, 255},  // neon magenta
            {120,  80, 255},  // violet-blue
            { 64, 140, 255},  // electric blue
            {  0, 220, 255}   // cyan
        };
        print_per_char_gradient(LOGO, FUTURE);

        // Taglines (no emoji if your font struggles—keep plain text)
        const string t1 = "    Secure Secret Management";
        const string t2 = "    Fast • Secure • Human-Friendly";

        auto tgrad = make_gradient({ {200,220,255}, {120,180,255} }, std::max(t1.size(), t2.size()));
        auto print_tag = [&](const string& s){
            for (size_t i = 0; i < s.size(); ++i) {
                char ch = s[i];
                if (ch == ' ') { cout << ch; continue; }
                cout << ansi24(tgrad[i]) << ch << reset();
            }
            cout << "\n";
        };
        print_tag(t1);
        ...
        const char* ascii_logo =
            ...
            "           | |  |   | |  | |\n"
            "           |_|  |   |_|  |_|\n";
        cout << ansi24({120,180,255}) << ascii_logo << reset();
        cout << ansi24({180,200,220}) << "    Secure Secret Management\n" << reset();
        cout << ansi24({140,180,255}) << "    Fast - Secure - Human-Friendly\n\n" << reset();
    } else {
        // Plain ASCII (no color, no Unicode)
        cout <<
            "AK - Secret Management CLI\n\n"
            "        ________    __   __\n"
            "        |__  __|    | |  | \\\n"
            "           | |  |   | |__| |\n"
            "           | |  |   |  __  |\n"
            ...
            "    Fast - Secure - Human-Friendly\n\n";
    }

    // Onboarding tips (ASCII only so they’re always readable)
    cout <<
        "Tips for getting started:\n"
        "1. Set your first secret: ak set API_KEY\n"
        "2. Create profiles:       ak save prod API_KEY DB_URL\n"
        "3. Load into your shell:  ak load prod\n"
        "4. Import from .env:      ak import -p dev -f env -i .env --keys\n"
        "5. More help:             ak help\n\n"
...
int main(int argc, char **argv)
{
    ...
    string cmd = args.empty() ? "welcome" : args[0];
