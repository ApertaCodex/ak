...
std::pair<bool, std::string> curl_ok(const std::string& args) {
    std::string cmd = "curl -sS -f -L --connect-timeout 5 --max-time 12 " + args;
    std::string output = system::runCmdCapture(cmd);
    bool success = output.empty() == false; // Simplified check
