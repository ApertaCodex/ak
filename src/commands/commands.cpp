...
namespace ak
{
    namespace commands
    {
        ...
        int cmd_set(const core::Config &cfg, const std::vector<std::string> &args)
        {
            if (args.size() < 2)
            {
                core::error(cfg, "Usage: ak set <NAME> or ak set <NAME=VALUE>");
            }

            std::string input = args[1];
            std::string name, value;
            
            // Check if input contains '=' for NAME=VALUE format
            size_t equalPos = input.find('=');
            if (equalPos != std::string::npos)
            {
                // Parse NAME=VALUE format
                name = input.substr(0, equalPos);
                value = input.substr(equalPos + 1);
                
                if (name.empty())
                {
                    core::error(cfg, "Empty key name in NAME=VALUE format");
                }
            }
            else
            {
                // Traditional format - prompt for value
                name = input;
                value = system::promptSecret("Enter value for " + name + ": ");
            }
        ...
        int cmd_cp(const core::Config &cfg, const std::vector<std::string> &args)
        {
            ...
            if (!clearTime.empty())
            {
                ...
                {
                    // Execute system command in background - we don't need to check the result
                    // as this is a fire-and-forget clipboard clearing operation
                    int result = std::system(clearCommand.c_str());
                    (void)result; // Explicitly ignore the result since this is a background operation
                }
            }
            ...
            else if (isatty(STDIN_FILENO) && isatty(STDOUT_FILENO))
            {
                ...
                if (shell)
                {
                    ...
                    if (shellPath.find("bash") != std::string::npos)
                        currentShell = "bash";
                    else if (shellPath.find("zsh") != std::string::npos)
                        currentShell = "zsh";
                    else if (shellPath.find("fish") != std::string::npos)
                        currentShell = "fish";
                }
            }
        ...
        int cmd_test(const core::Config &cfg, const std::vector<std::string> &args)
        {
            ...
            (void)(std::find(args.begin(), args.end(), "--quiet") != args.end()); // quiet - suppressed unused warning
                        ...
                        else
                        {
                            ...
                            try
                            {
                                ...
                                for (const auto &customService : customServices)
                                {
                                    ...
                                    {
                                        serviceName = customService.description.empty() ? customService.name : customService.description;
                                        break;
                                    }
                                }
                            }
                        ...
                        else
                        {
                            ...
                            try
                            {
                                ...
                                for (const auto &customService : customServices)
                                {
                                    if (customService.name == result.service)
                                    {
                                        displayName = customService.description.empty() ? customService.name : customService.description;
                                        break;
                                    }
                                }
                            }
            ...
            if (subcommand == "list" || subcommand == "ls")
            {
                try
                {
                    ...
                    for (const auto &[name, service] : services::DEFAULT_SERVICES)
                    {
                        ...
                        std::string testable = service.testable ? ui::colorize(" (testable)", ui::Colors::BRIGHT_GREEN) : ui::colorize(" (read-only)", ui::Colors::DIM);
