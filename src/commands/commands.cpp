...
int cmd_load(const core::Config& cfg, const std::vector<std::string>& args) {
    ...
    if (isInteractive && hasShellIntegration) {
        ...
        std::cerr << "💡 " << ui::colorize("DIRECT LOADING:", ui::Colors::BRIGHT_GREEN) << " Use " << ui::colorize("ak_load " + name + (persist ? " --persist" : ""), ui::Colors::BRIGHT_CYAN) << "\n";
        std::cerr << "   " << ui::colorize("ALTERNATIVE:", ui::Colors::BRIGHT_YELLOW) << " " << ui::colorize("eval \"$(ak load " + name + (persist ? " --persist" : "") + ")\"", ui::Colors::BRIGHT_YELLOW) << "\n";
        ...
        std::cerr << "   " << ui::colorize("FOR EASIER USE:", ui::Colors::BRIGHT_YELLOW) << " Install shell integration with " << ui::colorize("ak install-shell", ui::Colors::BRIGHT_YELLOW) << "\n";
    } else {
