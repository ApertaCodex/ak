...
int cmd_test(const core::Config& cfg, const std::vector<std::string>& args) {
    ...
    if (!profileName.empty()) {
        ...
        // Test specific services: ./ak test openai anthropic
    ...
    if (subcommand == "list" || subcommand == "ls") {
        try {
            ...
            if (customServices.empty()) {
                core::info(cfg, "No custom services configured. Use 'ak service add' to create one.");
                return 0;
            }
            
            std::cout << ui::colorize("🔧 Custom Services:", ui::Colors::BRIGHT_MAGENTA) << "\n";
            ...
            for (const auto& service : customServices) {
                ...
                std::string testable = service.testable ? 
                    ...
                    ui::colorize(" (not testable)", ui::Colors::DIM);
            ...
            std::cout << "\n" << ui::colorize("Total: " + std::to_string(customServices.size()) + " custom services", ui::Colors::DIM) << "\n";
            
        } catch (const std::exception& e) {
            core::error(cfg, "Failed to load custom services: " + std::string(e.what()));
        }
        ...
        try {
            ...
            std::cout << ui::colorize("Enable testing? [" + (editedService.testable ? "Y/n" : "y/N") + "]: ", ui::Colors::WHITE);
            std::getline(std::cin, input);
