    ...
    if (cfg.json) {
        ...
        for (const auto& result : results) {
            if (servicesToTest.size() == 1) {
                ...
                std::cout << ui::colorize("├── Validating API key format...", ui::Colors::DIM) << " ";
                std::cout << ui::colorize("✓", ui::Colors::BRIGHT_GREEN) << "\n";
                std::cout << ui::colorize("├── Checking API endpoint...", ui::Colors::DIM) << " ";
                std::cout << ui::colorize("✓", ui::Colors::BRIGHT_GREEN) << "\n";
                std::cout << ui::colorize("└── Testing authentication...", ui::Colors::DIM) << " ";
                ...
                // Multiple services - show compact results
                if (result.ok) {
