...
void ProfileManagerWidget::onProfileSelectionChanged()
{
    ...
    if (hasSelection) {
        ...
        // Emit signal to notify other widgets (like KeyManager) about the profile selection
        emit profileSelectionChanged(profileName);
    } else {
        updateProfileDetails("");
        // Emit empty string to indicate no profile selected
