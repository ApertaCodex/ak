...
// Custom event IDs
enum {
    ID_StatusMessage = wxID_HIGHEST + 1000
};
...
MaskedCellRenderer::MaskedCellRenderer(KeyManagerWidget* widget)
    : wxGridCellStringRenderer(), widget(widget)
{
}
                             ...
                             const wxRect& rect, int row, int col, bool isSelected)
{
    ...
    // Make sure we have a reference to our widget
    if (!widget) {
        widget = dynamic_cast<KeyManagerWidget*>(grid.GetParent());
    }
    
    if (widget) {
        // Check if this is a masked cell and if it should be displayed masked
        wxString keyName = grid.GetCellValue(row, KeyManagerWidget::ColName);
        ...
        if (isMasked && col == KeyManagerWidget::ColValue) {
...
void KeyManagerWidget::SetupGrid()
{
    ...
    grid->SetCellRenderer(0, ColValue, new MaskedCellRenderer(this));
    ...
    grid->Bind(wxEVT_GRID_CELL_LEFT_DCLICK, [this](wxGridEvent& WXUNUSED(event)) {
        wxCommandEvent cmdEvent(wxEVT_COMMAND_BUTTON_CLICKED, ID_EDIT_KEY);
        OnEditKey(cmdEvent);
    });
}
...
void KeyManagerWidget::UpdateGrid()
{
    ...
    if (grid->GetNumberRows() > 0) {
        grid->SelectRow(0);
        wxGridEvent evt(grid->GetId(), wxEVT_GRID_SELECT_CELL, grid, 0, 0);
        OnGridCellSelected(evt);
    } else {
        editButton->Enable(false);
        deleteButton->Enable(false);
        toggleVisibilityButton->Enable(false);
                                   ...
                                   const wxString &service, const wxString &apiUrl)
{
    ...
    if (!apiUrl.IsEmpty()) {
        ...
        // wxWidgets doesn't support direct cell tooltips, so we'll use a different approach
        // We could implement custom cell rendering or use wxGrid::GetGridWindow()->SetToolTip in the mouse event
        grid->SetAttr(row, ColUrl, attr);
    }
    ...
    grid->SetAttr(row, ColTestStatus, testStatusAttr);
...
wxString KeyManagerWidget::GetServiceApiUrl(const wxString &service)
{
    ...
    return it != serviceUrls.end() ? it->second : wxString();
}
...
void KeyManagerWidget::OnContextMenu(wxContextMenuEvent& WXUNUSED(event))
{
...
void KeyManagerWidget::UpdateTestStatus(const wxString &keyName, bool success, const wxString &message)
{
    ...
    if (row >= 0) {
        ...
        grid->SetAttr(row, ColTestStatus, attr);
