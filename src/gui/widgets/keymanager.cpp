...
void KeyManagerWidget::setupTable()
{
    ...
    // Configure column widths - allow manual resizing
    QHeaderView *header = table->horizontalHeader();
    header->setSectionResizeMode(QHeaderView::Interactive);
    
    // Set initial column widths
    header->resizeSection(ColumnName, 200);
    header->resizeSection(ColumnService, 120);
    header->resizeSection(ColumnUrl, 200);
    header->resizeSection(ColumnValue, 250);
    header->resizeSection(ColumnTestStatus, 100);
    header->resizeSection(ColumnActions, 80);
    
    // Allow stretching of the last section
