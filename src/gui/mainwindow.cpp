...
void MainWindow::run()
{
    ...
    try {
        std::cerr << "DEBUG: Showing window" << std::endl;
        
        // Show window without running event loop directly
        ui->show();
        
        std::cerr << "DEBUG: Running slint::run_event_loop()" << std::endl;
        
        // Use the documented API for running the event loop
        slint::run_event_loop();
        
        std::cerr << "DEBUG: Event loop returned" << std::endl;
...
void MainWindow::exitApplication()
{
    ...
    // In Slint 1.13.0, use hide() which will cause run() to return
    ui->hide();
}
