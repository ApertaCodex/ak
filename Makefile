...
TEST_OBJS := $(OBJDIR)/tests/test_main.o $(patsubst tests/%.cpp,$(OBJDIR)/tests/%.o,$(TEST_UNIT_SRCS))
...
$(OBJDIR)/tests/test_main.o: tests/test_main.cpp
	@mkdir -p $(@D)
	$(CXX) $(CXXFLAGS) -Itests -DCATCH_CONFIG_IMPL_ONLY -c $< -o $@

$(OBJDIR)/tests/test_main.o: tests/test_main.cpp
	@mkdir -p $(@D)
	$(CXX) $(CXXFLAGS) -Itests -DCATCH_CONFIG_IMPL_ONLY -c $< -o $@

$(OBJDIR)/tests/core/test_config.o: tests/core/test_config.cpp
	@mkdir -p $(@D)
	$(CXX) $(CXXFLAGS) -Itests -c $< -o $@

$(OBJDIR)/tests/crypto/test_crypto.o: tests/crypto/test_crypto.cpp
	@mkdir -p $(@D)
	$(CXX) $(CXXFLAGS) -Itests -c $< -o $@

$(OBJDIR)/tests/cli/test_cli.o: tests/cli/test_cli.cpp
	@mkdir -p $(@D)
	$(CXX) $(CXXFLAGS) -Itests -c $< -o $@

$(OBJDIR)/tests/services/test_services.o: tests/services/test_services.cpp
	@mkdir -p $(@D)
	$(CXX) $(CXXFLAGS) -Itests -c $< -o $@
