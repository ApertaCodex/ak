...
# Object files for Catch2 amalgamation
CATCH_IMPL_SRC := tests/catch_impl.cpp
CATCH_OBJ := $(patsubst tests/%.cpp,$(OBJDIR)/tests/%.o,$(CATCH_IMPL_SRC))
...
$(OBJDIR)/tests/catch_impl.o: tests/catch_impl.cpp
	@mkdir -p $(@D)
...
$(TEST_BIN): $(TEST_OBJS) $(MODULE_OBJS) $(CATCH_OBJ)
	$(CXX) $(LDFLAGS) $^ -o $@
