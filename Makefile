...
coverage: clean-coverage
	...
	@if which lcov >/dev/null 2>&1; then \
		lcov --capture --directory obj/src --output-file coverage.info --ignore-errors path --quiet; \
		lcov --remove coverage.info '/usr/*' '*/tests/*' --output-file coverage.info --ignore-errors path --quiet; \
