...
coverage: clean-coverage
	...
	@cd obj && gcov -r src/*/*.gcno >/dev/null 2>&1 || true
	@mv obj/*.gcov . 2>/dev/null || true
...
coverage-summary: coverage
	...
	@echo "===================="
	@find . -name "*.gcov" -exec basename {} .gcov \; | sort | uniq | while read file; do \
		if [ -f "$$file.gcov" ]; then \
			lines_total=$$(grep -c ":" "$$file.gcov" || echo 0); \
			lines_covered=$$(grep -c "^[ ]*[0-9][ ]*[0-9]" "$$file.gcov" || echo 0); \
			if [ $$lines_total -gt 0 ]; then \
				coverage_pct=$$(echo "scale=1; $$lines_covered * 100 / $$lines_total" | bc -l 2>/dev/null || echo "0.0"); \
				printf "%-30s: %3s/%3s lines (%s%%)\n" "$$file" "$$lines_covered" "$$lines_total" "$$coverage_pct"; \
			fi; \
		fi; \
	done
	@echo "===================="
