...
#   make release        # minor version bump + test + commit + tag + publish to GitHub Pages
#   make release-patch  # patch version bump + test + commit + tag + publish to GitHub Pages
#   make release-major  # major version bump + test + commit + tag + publish to GitHub Pages
#   VERSION_BUMP=patch make release     # override default (minor) with patch
#   INCLUDE_PPA=yes make release        # optionally include Launchpad PPA upload
#   make clean
...
publish-all:
	...
	@$(MAKE) publish-apt
	@echo "📦 2/2 Building distribution packages..."
	@$(MAKE) dist || (echo "⚠️  Package build failed - continuing"; true)
	@if [ "$(INCLUDE_PPA)" = "yes" ]; then \
		echo "📦 Optional: Publishing to Launchpad PPA..."; \
		$(MAKE) publish-ppa || (echo "⚠️  PPA publish failed - continuing"; true); \
	fi
